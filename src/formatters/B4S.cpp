...
bool Exporter::formatB4S(QTextStream &stream)
{
    ...
    stream << "<WinampXML>" << NEWLINE;
    stream << TABCHAR << QString("<playlist num_entries=\"%1\" label=\"%2\">").arg(QString::number(Playlist.size()), suitForXML(PlaylistTitle)) << NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        stream << TABCHAR << TABCHAR << QString("<entry Playstring=\"%1\">").arg(suitForXML(filePath)) << NEWLINE;
        stream << TABCHAR << TABCHAR << TABCHAR << QString("<Name>%1</Name>").arg(suitForXML(title)) << NEWLINE;
        stream << TABCHAR << TABCHAR << "</entry>" << NEWLINE;
    }
