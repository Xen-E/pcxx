...
bool Exporter::formatWPL(QTextStream &stream)
{
    ...
    stream << TABCHAR << TABCHAR << QString("<title>%1</title>").arg(suitForXML(PlaylistTitle)) << NEWLINE;
    ...
    for (auto &track : Playlist)
        totalDuration += track.rawLength;
    stream << TABCHAR << TABCHAR << QString("<meta name=\"TotalDuration\" content=\"%1\" />").arg(QString::number(totalDuration)) << NEWLINE;

    stream << TABCHAR << TABCHAR << QString("<meta name=\"ItemCount\" content=\"%1\" />").arg(QString::number(Playlist.size())) << NEWLINE;
    stream << TABCHAR << "</head>" << NEWLINE;
    stream << TABCHAR << "<body>" << NEWLINE;
    stream << TABCHAR << TABCHAR << "<seq>" << NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        stream << TABCHAR << TABCHAR << TABCHAR << QString("<media src=\"%1\" />").arg(suitForXML(filePath)) << NEWLINE;
    }
