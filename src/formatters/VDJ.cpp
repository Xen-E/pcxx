...
bool Exporter::formatVDJ(QTextStream &stream)
{
    ...
    for (auto &track : Playlist) {
        ...
        const QString extvdj(QString(
        "#EXTVDJ:<filesize>%1</filesize><artist>%2</artist><title>%3</title><songlength>%4</songlength>"
        ).arg(QString::number(track.rawFileSize), artist, title, QString::number(track.rawLength)));

        stream << extvdj << NEWLINE;
        stream << filePath;
