...
bool Exporter::formatMPCPL(QTextStream &stream)
{
    QStringList failedPathPrefix; QStringList missingFiles;

    stream << "MPCPLAYLIST" + NEWLINE + NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        const QString typeLine(QString("%1,type,0").arg(QString::number(index)));
        const QString labelLine(QString("%1,label,%2").arg(QString::number(index), title));
        const QString fileLine(QString("%1,filename,%2").arg(QString::number(index), filePath));

        stream << typeLine << NEWLINE;
        stream << labelLine << NEWLINE;
        stream << fileLine << NEWLINE << NEWLINE;
    }
