...
bool Exporter::formatM3U(QTextStream &stream)
{
    QStringList failedPathPrefix; QStringList missingFiles;

    stream << "#EXTM3U" << NEWLINE;
    stream << QString("#EXTENC: %1").arg(Codec) << NEWLINE;
    stream << QString("#PLAYLIST: %1").arg(PlaylistTitle) << NEWLINE << NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        const QString extinf(QString("#EXTINF:%1, %2 - %3").arg(QString::number(track.rawLength), artist, title));
        stream << extinf << NEWLINE;
        stream << filePath;
