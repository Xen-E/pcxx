...
bool Exporter::formatASX(QTextStream &stream)
{
    ...
    stream << "<asx version=\"3.0\">" << NEWLINE;
    stream << TABCHAR << QString("<title>%1</title>").arg(suitForXML(PlaylistTitle)) << NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        stream << TABCHAR << "<entry>" << NEWLINE;
        stream << TABCHAR << TABCHAR << QString("<author>%1</author>").arg(suitForXML(artist)) << NEWLINE;
        stream << TABCHAR << TABCHAR << QString("<title>%1</title>").arg(suitForXML(title)) << NEWLINE;
        stream << TABCHAR << TABCHAR << QString("<duration value=\"%1\" />").arg(duration) << NEWLINE;
        stream << TABCHAR << TABCHAR << QString("<ref href=\"%1\" />").arg(suitForXML(filePath)) << NEWLINE;
        stream << TABCHAR << "</entry>" << NEWLINE;
    }
