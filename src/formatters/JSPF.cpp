...
bool Exporter::formatJSPF(QTextStream &stream)
{
    QStringList failedPathPrefix; QStringList missingFiles;

    stream << "{" << NEWLINE;
    stream << TABCHAR << "\"playlist\" : {" << NEWLINE;
    stream << TABCHAR << TABCHAR << QString("\"title\" : \"%1\",").arg(PlaylistTitle) << NEWLINE;
    stream << TABCHAR << TABCHAR << QString("\"creator\" : \"%1 V%2\",").arg(Product::name, Product::version) << NEWLINE;
    ...
    for (auto &track : Playlist) {
        ...
        stream << TABCHAR << TABCHAR << TABCHAR << QString("\"creator\" : \"%1\",").arg(artist) << NEWLINE;
        stream << TABCHAR << TABCHAR << TABCHAR << QString("\"title\" : \"%1\",").arg(title) << NEWLINE;
        stream << TABCHAR << TABCHAR << TABCHAR << QString("\"album\" : \"%1\",").arg(album) << NEWLINE;
        stream << TABCHAR << TABCHAR << TABCHAR << QString("\"duration\" : \"%1\",").arg(duration) << NEWLINE;
        stream << TABCHAR << TABCHAR << TABCHAR << QString("\"location\" : [\"%1\"]").arg(QDir::fromNativeSeparators(filePath)) << NEWLINE;

        QString lastTrack = (&track - Playlist.begin()) == Playlist.size() ? "}" : "},";
        stream << TABCHAR << TABCHAR << TABCHAR << lastTrack << NEWLINE;
    }

    stream << TABCHAR << TABCHAR << "]" << NEWLINE;
    stream << TABCHAR << "}" << NEWLINE;
    stream << "}";
